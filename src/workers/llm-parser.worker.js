...
import { LOCAL_LLM_MODELS, LLM_DTYPE_CHAIN, LLM_DTYPE_CHAIN_WEBGPU, } from '@/lib/llm-parser';
 ...
 */
const GPU_WARMUP_TIMEOUT_MS = 45000;
const GPU_WARMUP_TIMEOUT_MESSAGE = 'WebGPU stalled at first inference — the model loaded but a 2-token warmup generation did not finish in 45s. ' +
 ...
 */
let memReportedAt = 0;
function postMemStats(force = false) {
    const now = performance.now();
    if (!force && now - memReportedAt < 2000)
        return;
    memReportedAt = now;
    const memory = self
        .performance?.memory;
    if (!memory)
        return; // non-Chromium: API unavailable, badge just stays hidden
    post({
        type: 'mem-stats',
 ...
 */
async function loadModel(model, preferred, deviceRequest) {
    ...
    const dtypeOrder = (device) => {
        const wasmOrder = preferred
            ...
            try {
                ...
                if (device === 'webgpu') {
                    const warmed = await Promise.race([
                        pipe([{ role: 'user', content: 'Say OK.' }], { max_new_tokens: 2, do_sample: false, return_full_text: false }).then(() => true),
                        new Promise((resolve) => {
    ...
    post({
        type: 'load-error',
        model,
        message: `Could not load ${model} in any precision. Last error: ${failedAttempts[failedAttempts.length - 1]?.message ?? 'unknown'}`,
        ...(failedAttempts.length > 0 ? { failedAttempts } : {}),
    });
}
async function runParse(job) {
    ...
    try {
        ...
        if (loading)
            await loading;
        if (!current)
            throw new Error('No LLM model is loaded yet.');
...
scope.addEventListener('message', (event) => {
    ...
    if (data.type === 'parse') {
        ...
        parseChain = parseChain
            .then(() => runParse({ id: data.id, system: data.system, user: data.user, maxNewTokens: data.maxNewTokens }))
            .catch(() => undefined);
    }
});
