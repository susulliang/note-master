...
import { LOCAL_WHISPER_MODELS, DTYPE_CHAIN, } from '@/lib/whisper-models';
 ...
 */
async function loadModel(model, preferred) {
    ...
    post({
        type: 'load-error',
        model,
        message: `Could not load ${model} in any precision. Last error: ${lastError?.message ?? String(lastError)}`,
    });
}
async function handleTranscribe(id, audio) {
    ...
    try {
        ...
        if (loading)
            await loading;
        if (!current)
            throw new Error('No Whisper model is loaded yet.');
