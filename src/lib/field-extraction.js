 ...
 */
function normalizeModelKeyZeroAsO(raw) {
 ...
 */
const ASR_ARTIFACT_TAG = /\[\s*(?:inaudible|blank[_ ]?audio|silence|noise|music(?:\s+playing)?|applause|laughter|crosstalk|unintelligible|no speech|pause)\s*\]/gi;
/** Remove ASR artifact tags from a transcript turn */
export function stripAsrArtifacts(text) {
    return text.replace(ASR_ARTIFACT_TAG, ' ').replace(/\s{2,}/g, ' ').trim();
}
/** True when a turn is ONLY ASR artifacts (no actual speech in it) */
export function isAsrArtifact(text) {
    return stripAsrArtifacts(text).length === 0;
}
export function matchCanonicalModel(raw) {
    const key = normalizeModelKey(raw);
    if (!key || key.length < 2)
        return null;
    // Exact match first
    for (const option of DEEBOT_MODELS) {
        if (normalizeModelKey(option) === key)
            return option;
    }
    ...
    for (const option of DEEBOT_MODELS) {
        ...
        if (key.length >= 3 && optKey.startsWith(key) ||
            ...
            if (!best || normalizeModelKey(best).length > optKey.length)
                best = option;
        }
    }
    if (best)
    ...
    for (const option of DEEBOT_MODELS) {
        ...
        if (okey.length >= 3 && optKey.startsWith(okey) ||
            (okey.length >= 4 && optKey.endsWith(okey))) {
            if (!best || normalizeModelKeyZeroAsO(best).length > optKey.length)
                best = option;
        }
    }
    return best;
...
export function canonicalIssueType(raw) {
    const key = raw.toLowerCase().replace(/[^a-z0-9]/g, '');
    if (!key)
        return null;
    for (const option of ISSUE_TYPES) {
        if (option.toLowerCase().replace(/[^a-z0-9]/g, '') === key)
            return option;
    }
    let best = null;
    for (const option of ISSUE_TYPES) {
 ...
 */
export function summarizeIssueType(description) {
...
export function canonicalPurchaseChannel(raw) {
 ...
 * same way.
 */
export function formatPurchaseValue(raw) {
    ...
    const words = rest
        ...
        .slice(0, 8);
    while (words.length > 0 && WHEN_GLUE.has(words[0].toLowerCase()))
        words.shift();
    while (words.length > 0 && WHEN_GLUE.has(words[words.length - 1].toLowerCase()))
        words.pop();
...
function extractModelValue(match) {
 ...
 */
function collectAccumulated(text, patterns) {
    const results = [];
    const seen = new Set();
    for (const pattern of patterns) {
        for (const match of text.matchAll(pattern)) {
            ...
            if (results.some((r) => r.toLowerCase().includes(key)))
                continue;
            const subsumes = results.findIndex((r) => key.includes(r.toLowerCase()));
            if (subsumes >= 0)
                results.splice(subsumes, 1);
            seen.add(key);
            results.push(value);
        }
    }
 ...
 */
export const ACCUMULATING_FIELD_IDS = new Set(FIELD_PATTERNS.filter((p) => p.accumulate).map((p) => p.fieldId).concat('issueType'));
/**
 ...
 */
export function extractFields(entries) {
    if (entries.length === 0)
        return [];
    ...
    for (const entry of FIELD_PATTERNS) {
        if (seenFields.has(entry.fieldId))
            continue;
        if (entry.fieldId === 'purchaseInfo') {
            ...
            const pools = [];
            if (customerText)
                pools.push({ text: customerText, patterns: entry.customer ?? [] });
            if (agentText)
                pools.push({ text: agentText, patterns: entry.agent ?? [] });
            ...
            const best = candidates.find((c) => canonicalPurchaseChannel(c)) ?? candidates[0];
            if (best) {
                ...
                const noChannelJunk = /^(?:him|her|them|there|here|somebody|someone|anybody|you|me|us|that|this guy|the guy|a guy|my husband|my wife|my son|my daughter)\b/i;
                if (value.length > 2 &&
        ...
        for (const pool of pools) {
            for (const pattern of pool.patterns) {
                ...
                if (value.length <= 2)
                    continue;
                const confidence = entry.fieldId === 'emailAddress' ||
