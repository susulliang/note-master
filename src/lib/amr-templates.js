...
function stem(token) {
    ...
    if (s.length > 5 && s.endsWith('ing'))
        s = s.slice(0, -3);
    else if (s.length > 3 && s.endsWith('s') && !/(ss|us|is)$/.test(s))
        s = s.slice(0, -1);
    return s;
}
...
function extractErrorCode(file) {
    const m = file.match(/(?:^|_)(\d{3,4})(?=_|$)/);
    return m && m[1].length >= 3 ? m[1] : null;
}
function makeEntry(path, raw, kind, category, nameOverride) {
...
export const ALL_TEMPLATES = [
    ...Object.entries(rawModules).map(([path, raw]) => makeEntry(path, raw, 'amr', 'AMR Email')),
    ...Object.entries(macroModules)
        .filter(([path]) => !/INDEX/i.test(path))
        .map(([path, raw]) => makeEntry(path, raw, 'tbs', path.includes('/general/') ? 'TBS · General' : 'TBS')),
    ...
    ...Object.entries(productFaqModules)
        ...
        const display = (fmModel && h1 && fmModel.toLowerCase().includes(h1.toLowerCase().slice(0, 10)) === false)
...
for (const group of SYNONYM_GROUPS) {
    ...
    for (const s of stems) {
        if (!SYNONYMS.has(s))
            SYNONYMS.set(s, new Set());
        for (const other of stems) {
            if (other !== s)
                SYNONYMS.get(s).add(other);
        }
    }
}
 ...
 */
export function searchTemplates(query, limit = 8) {
 ...
 */
function parseMarkdown(md, fallbackName) {
    const lines = [];
    let title = '';
    for (const rawLine of md.split(/\r?\n/)) {
        ...
        if (/^!\[[^\]]*\]\([^)]*\)$/.test(line))
            continue;
        if (/^\[.*\.(mp4|mov|avi|mkv)\]$/i.test(line))
            continue;
 ...
 */
export function parseTemplate(entry) {
    if (entry.kind === 'amr') {
...
function parseAmrHtml(html) {
    const doc = new DOMParser().parseFromString(html, 'text/html');
    doc.querySelectorAll('script, style').forEach((el) => el.remove());
    const meta = [];
    const lines = [];
    const cleanText = (el) => (el?.textContent ?? '').replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim();
    // Push each <br>-separated segment of an element as its own line
    const pushBrLines = (el) => {
