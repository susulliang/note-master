 ...
 */
export const DEFAULT_LLM_MODEL = 'qwen2.5-0.5b';
export const LLM_MODEL_META = {
    'smollm2-360m': {
        label: 'SmolLM2 360M',
...
export const LLM_MODELS = Object.keys(LOCAL_LLM_MODELS);
/**
 ...
 */
export const LLM_RAM_ESTIMATE_MB = {
 ...
 */
export const LLM_DTYPE_CHAIN = ['q8', 'fp32'];
/**
 ...
 */
export const LLM_DTYPE_CHAIN_WEBGPU = ['q4f16', 'fp16', 'fp32'];
/**
 ...
 */
export function describeLoadError(raw) {
...
export function readLlmModelPref() {
    try {
        const value = localStorage.getItem(LLM_MODEL_PREF_KEY);
        if (value && value in LOCAL_LLM_MODELS)
            return value;
    }
    catch {
...
export function writeLlmModelPref(model) {
    try {
...
export function readLlmEnabledPref() {
    try {
...
export function writeLlmEnabledPref(enabled) {
    try {
 ...
 */
const MAX_TRANSCRIPT_CHARS_CPU = 2500;
/** Transcript-window cap for the backend the parser is running on. */
export function getTranscriptCharCap(device) {
...
const FILLER_ONLY_TURN = /^(?:you|u|yeah|yep|yup|ya|okay|ok|hm+|mhm+|mm+|uh+[- ]?huh+|oh?k?ay|alright|right|sure|great|perfect|awesome|cool|wow)\b[.!]??$/i;
 ...
 */
export function buildPromptWindow(entries, maxChars = MAX_TRANSCRIPT_CHARS) {
    const kept = [];
    for (let i = 0; i < entries.length; i += 1) {
        const e = entries[i];
        if (isNoiseTurn(e.text))
            continue;
    ...
    if (chars > maxChars) {
        ...
        for (let i = kept.length - 1; i >= 0; i -= 1) {
            ...
            if (chars + len > maxChars && lines.length > 0)
                break;
...
export function renderTranscript(entries, maxChars = MAX_TRANSCRIPT_CHARS) {
...
export function buildParsePrompt(entries, missingFieldIds, prior, strict = false, 
/**
...
maxChars = MAX_TRANSCRIPT_CHARS) {
    const wanted = missingFieldIds.filter((id) => LLM_FIELD_IDS.includes(id));
    const order = wanted.length > 0
        ? PROMPT_FIELD_ORDER.filter((id) => wanted.includes(id))
        : [...PROMPT_FIELD_ORDER];
    ...
    const skeleton = Object.fromEntries(order.map((id) => [
        id,
    ...
    const userLines = ['Support call transcript:', renderTranscript(entries, maxChars)];
    if (prior?.issueDescription) {
        userLines.push('', 'Issue description clauses already on the ticket (keep EVERY one of them; append NEW points the customer describes or the agent confirms — never drop a clause):', prior.issueDescription);
    }
    if (prior?.resolutionSummary) {
        userLines.push('', 'Steps already on the ticket (keep them unchanged and in order, then append any NEW steps after them):', prior.resolutionSummary);
    }
    userLines.push('', `Extract these ticket fields as JSON ("" when unknown; the two fields already hold the ticket's current value — keep ALL of it and append any NEW clauses from the transcript): ${JSON.stringify(skeleton)}`);
    return { system, user: userLines.join('\n') };
}
/**
 ...
 */
export function buildParaphrasePrompt(input) {
    const system = [
 ...
 */
export function validateParaphraseReply(raw) {
    const out = [];
    const keys = ['issueDescription', 'resolutionSummary'];
 ...
 */
export function extractJsonLoose(raw) {
...
export function extractLineFields(raw) {
    if (!raw)
        return null;
    const out = {};
...
function isPlaceholderFor(fieldId, value) {
 ...
 */
export function validateLlmFields(raw) {
    const out = [];
    for (const fieldId of LLM_FIELD_IDS) {
        ...
        switch (fieldId) {
            ...
            case 'contactNumber': {
                ...
                if (digitCount < 7 && isPlaceholderFor('contactNumber', rawBeforeClean))
                    continue;
                break;
            }
            case 'emailAddress': {
            ...
            case 'serialNumber': {
                ...
                const placeholderKey = fieldId;
                if (isPlaceholderFor(placeholderKey, cleaned))
                    continue;
                break;
            }
            case 'issueType': {
                ...
                const canonical = canonicalIssueType(cleaned) ?? classifyIssueType(cleaned);
        ...
        if (cleaned.length > cap)
            cleaned = cleaned.slice(0, cap).trimEnd();
        if (cleaned.length === 0)
            continue;
