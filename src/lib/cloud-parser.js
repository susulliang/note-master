 ...
 * differs.
 */
import { buildParsePrompt, extractJsonLoose, extractLineFields, validateLlmFields, } from '@/lib/llm-parser';
 ...
 */
export function isDeepSeekDefault() {
    try {
...
export function writeDeepseekApiKey(key) {
    try {
        if (key)
 ...
 * literal "data: [DONE]\n\n".
 */
async function readSseStream(reader, decoder, onDelta) {
    let buffer = '';
    let acc = '';
    let done = false;
    while (!done) {
 ...
 */
export async function generateWithDeepseek(system, user, maxTokens = 512, onProgress) {
    const key = getEffectiveApiKey();
    const started = performance.now();
    onProgress?.(0, 'connecting');
    if (!key)
        return { text: '', ms: Math.round(performance.now() - started), timedOut: false };
    if (!system && !user)
        return { text: '', ms: 0, timedOut: false };
    let response;
    try {
 ...
 */
export async function parseWithDeepseek(apiKey, entries, prior, onProgress) {
    const key = apiKey || getEffectiveApiKey();
    if (!key)
        return { result: null, error: 'No DeepSeek API key stored.' };
    if (entries.length === 0)
        return { result: null, error: 'Nothing to parse — the transcript is empty.' };
    ...
    try {
        ...
        return {
            result: null,
            error: `Network error reaching DeepSeek: ${err.message}. Check your connection.`,
        };
    }
    if (!response.ok) {
        let detail = `${response.status} ${response.statusText}`;
    ...
    if (!raw)
        return { result: null, error: 'DeepSeek returned an empty reply.' };
