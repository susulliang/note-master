 ...
 */
function StandaloneAppContainer({ children }) {
    return <>{children}</>;
}
function StandaloneErrorRender({ error, resetErrorBoundary, }) {
...
createRoot(document.getElementById("root")).render(<StrictMode>
    ...
    globalThis.process?.env?.CLIENT_BASE_PATH ||
        import.meta.env?.BASE_URL ||
        "/"}>
      <StandaloneAppContainer>
        <ErrorBoundary fallbackRender={({ error, resetErrorBoundary }) => (<StandaloneErrorRender error={error} resetErrorBoundary={resetErrorBoundary}/>)}>
          <App />
        </ErrorBoundary>
      </StandaloneAppContainer>
    </BrowserRouter>
