...
import { User, Phone, Mail, MapPin, Barcode, Hash, Bot, TriangleAlert, FileText, CheckCircle2, StickyNote, ShoppingBag, FileSearch, } from 'lucide-react';
...
import { normalizeTheme, nextTheme, getThemeMeta, NARROW_SCREEN_WIDTH, } from '@/lib/themes';
import { DEEBOT_MODELS, ISSUE_TYPES, NODE_IDS, MAX_HISTORY_ENTRIES, RESOLUTION_QUICK_TEXTS, PURCHASE_QUICK_TEXTS, DETAILED_ISSUE_QUICK_TEXTS, FAILURE_TOP_ISSUES, HOWTO_TOP_ISSUES, } from '@/data/ticket';
...
// ---------------------------------------------------------------------------
function ExtensionPushConfirmPopup({ pending, labels, onApply, onDismiss, }) {
    ...
    return (<div className="fixed bottom-24 right-6 z-[70] w-[min(22rem,calc(100vw-3rem))]" role="dialog" aria-live="polite" aria-label="Scrape data received">
      <div className="glass-panel border border-border/80 rounded-lg shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/60">
          ...
          <button type="button" onClick={onDismiss} className="text-muted-foreground hover:text-foreground hover:bg-card/60 rounded px-1.5 py-1 text-xs transition" aria-label="Dismiss">
            ✕
          </button>
        </div>

        <dl className="px-4 py-3 space-y-2 max-h-64 overflow-y-auto">
          {fieldEntries.length === 0 && (<div className="text-xs text-muted-foreground italic py-2">No fields available.</div>)}
          {fieldEntries.map(([k, v]) => (<div key={k} className="grid grid-cols-[7rem_1fr] gap-3 items-start">
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-border/60 bg-card/40">
          <button type="button" onClick={onDismiss} className="px-3 py-1.5 rounded-md text-sm text-muted-foreground hover:text-foreground hover:bg-card/60 border border-border/60 transition">
            Dismiss
          </button>
          <button type="button" onClick={onApply} className="px-3 py-1.5 rounded-md text-sm font-medium text-primary-foreground bg-primary hover:brightness-110 border border-primary/60 shadow-sm transition">
            Apply to form
          </button>
        </div>
      </div>
...
export default function TicketNotesPage() {
    const [formData, setFormData] = useScopedState('ecovacs_ticket_form_data', INITIAL_FORM_DATA);
    ...
    const [positions, setPositions] = useScopedState('ecovacs_ticket_node_position_overrides', {});
    /**
     ...
     */
    const [hiddenNodeIds, setHiddenNodeIds] = useScopedState('ecovacs_ticket_hidden_nodes', []);
    const hiddenNodesSet = useMemo(() => new Set(hiddenNodeIds), [hiddenNodeIds]);
    ...
    const handleToggleGridbox = useCallback((id, nextVisible) => {
    ...
    const gridboxVisibilityToggles = useMemo(() => GRIDBOX_VISIBILITY_TOGGLES.map((t) => ({
        ...t,
        visible: !hiddenNodesSet.has(t.id),
    })), [GRIDBOX_VISIBILITY_TOGGLES, hiddenNodesSet]);
    const [rawTheme, setTheme] = useScopedState('ecovacs_ticket_theme', 'midnight');
    // Normalize legacy 'dark'/'light' values from older sessions
    const theme = normalizeTheme(rawTheme);
    // UI scale ("old people mode"): 1.25x zoom of the entire app
    const [uiScale, setUiScale] = useScopedState('ecovacs_ticket_ui_scale', 'normal');
    // Narrow screens auto-apply the small scale (one size down) unless the
    // user explicitly turned large mode on
    const [windowWidth, setWindowWidth] = useState(() => typeof window === 'undefined' ? NARROW_SCREEN_WIDTH + 1 : window.innerWidth);
    useEffect(() => {
        const onResize = () => setWindowWidth(window.innerWidth);
        window.addEventListener('resize', onResize);
        return () => window.removeEventListener('resize', onResize);
    }, []);
    const effectiveScale = uiScale === 'large' ? 'large' : windowWidth < NARROW_SCREEN_WIDTH ? 'small' : 'normal';
    const [history, setHistory] = useScopedState('ecovacs_ticket_notes_history', []);
    const [customResolutionQuickTexts, setCustomResolutionQuickTexts] = useScopedState('ecovacs_ticket_resolution_quicktexts', []);
    const [customPurchaseQuickTexts, setCustomPurchaseQuickTexts] = useScopedState('ecovacs_ticket_purchase_quicktexts', []);
    const [customIssueQuickTexts, setCustomIssueQuickTexts] = useScopedState('ecovacs_ticket_issue_quicktexts', []);
    ...
    const handleOpenTemplate = useCallback((tpl) => {
        setOpenTemplate(tpl);
    }, []);
    ...
    const pushUndo = useCallback((field, value, discrete) => {
        const now = Date.now();
        if (!discrete && now - lastUndoPush.current < UNDO_COALESCE_MS)
            return;
        const top = undoStack.current[undoStack.current.length - 1];
        if (top && top.field === field && top.value === value)
            return;
    ...
    const handleFieldChange = useCallback((id, value, discrete) => {
        if (UNDO_FIELDS.has(id) && typeof value === 'string') {
            const prev = formData[id];
            if (typeof prev === 'string')
                pushUndo(id, prev, Boolean(discrete));
        }
    ...
    // own undo step
    const handleInsertTemplateLine = useCallback((line) => {
        const current = typeof formData[NODE_IDS.RESOLUTION_SUMMARY] === 'string'
            ? formData[NODE_IDS.RESOLUTION_SUMMARY]
            : '';
        pushUndo(NODE_IDS.RESOLUTION_SUMMARY, current, true);
        setFormData((prev) => {
            const trimmed = typeof prev[NODE_IDS.RESOLUTION_SUMMARY] === 'string'
                ? prev[NODE_IDS.RESOLUTION_SUMMARY].trimEnd()
                : '';
            let next;
            if (!trimmed) {
                next = line;
    ...
    const handleResolutionChange = useCallback((text) => {
    ...
    const handleAddQuickText = useCallback((target, text) => {
        const t = text.trim();
        if (!t)
            return;
        ...
        if (target === 'resolution')
            ...
            setCustomIssueQuickTexts((prev) => [...prev, t]);
    ...
    const handleRemoveQuickText = useCallback((target, text) => {
        const remove = (prev) => prev.filter((t) => t !== text);
        if (target === 'resolution')
            setCustomResolutionQuickTexts(remove);
        else if (target === 'purchase')
            setCustomPurchaseQuickTexts(remove);
        else
            setCustomIssueQuickTexts(remove);
    ...
    const nodes = useMemo(() => NODES.map((n) => {
        ...
        const base = {
            ...
            onAddQuickText: (t) => handleAddQuickText(target, t),
            onRemoveQuickText: (t) => handleRemoveQuickText(target, t),
        };
        if (target === 'issue') {
            return {
                ...base,
    ...
    const buildNoteText = useCallback((data) => {
        const getStr = (key) => {
            const v = data[key];
            return typeof v === 'string' ? v : '';
        };
...
Additional information (if needed): ${getStr(NODE_IDS.ADDITIONAL_NOTES) || 'N/A'}`;
    ...
    const handleOutputClose = useCallback((finalText) => {
        const getStr = (key) => {
            const v = formData[key];
            return typeof v === 'string' ? v : '';
        };
     ...
     */
    const handleAutoFill = useCallback((fieldId, value, source) => {
        ...
        if (source === 'regex' && curTrimmed.length > 0)
            return;
        ...
        setFormData((prev) => {
            ...
            const merged = latest === curTrimmed
                ? plan
                : mergeAutoFill(latest, priorSource, base, value, source);
            if (merged.next === latest)
                return prev;
        ...
        if (plan.base !== null)
            llmBasesRef.current[nodeId] = plan.base;
        // 'regex-grow' is the same engine as 'regex' (one badge); the
        // paraphrase polish gets its own marker
        const storedSource = source === 'regex-grow' ? 'regex' : source;
        setParsedFields((prev) => prev[nodeId] === storedSource ? prev : { ...prev, [nodeId]: storedSource });
        if (plan.next !== curTrimmed) {
            const label = source === 'llm'
                ? 'AI parsed'
                : source === 'paraphrase'
                    ? 'AI polished'
                    : source === 'regex-grow'
    ...
    const autoFillRef = useRef(handleAutoFill);
    useEffect(() => {
    ...
    const sopCloudGenerate = useCallback((system, user, maxNewTokens = 512) => generateWithDeepseek(system, user, maxNewTokens), []);
        ...
        onApply: (mapped, _meta, _raw) => {
            ...
            for (const [nodeId, value] of Object.entries(mapped)) {
                ...
                try {
                    ...
                    const knownFieldKey = Object.entries(FIELD_TO_NODE).find(([, v]) => v === nodeId)?.[0] ?? nodeId;
    ...
    const handleSwitchWhisperModel = useCallback((model) => {
        localWhisper.switchModel(model);
    }, [localWhisper]);
    const handleSwitchLlmModel = useCallback((model) => {
        llmParser.switchModel(model);
    }, [llmParser]);
    const handleToggleLlmEnabled = useCallback((enabled) => {
        llmParser.setEnabled(enabled);
        if (enabled && !llmParser.isReady)
            void llmParser.load();
    }, [llmParser]);
    ...
    const handleLoadLlmDevice = useCallback((model, device) => {
    ...
    <div className="relative h-full w-full overflow-hidden bg-background font-sans text-foreground">
      ...
      <div className="flex h-full w-full">
        ...
        <main className="relative flex-1 min-w-0">
          <FloatingControls theme={theme} onCycleTheme={handleCycleTheme} onReset={handleReset} historyOpen={showHistory} onToggleHistory={handleToggleHistory} history={history} onDeleteHistory={handleDeleteHistory} onClearHistory={handleClearHistory} uiScale={uiScale} onToggleUiScale={handleToggleUiScale} callSupported={call.isSupported} callCapturing={call.isCapturing} onToggleCall={handleToggleCall} engine={{
        ...
        }} transcript={call.transcript} isTranscribing={call.isTranscribing} gridboxVisibility={{
            toggles: gridboxVisibilityToggles,
            onToggle: handleToggleGridbox,
        }}/>
          <TicketPanelsContext.Provider value={{
            transcriptContent: (<VoiceCaptionPanel mic={{
            ...
            sopContent: (<SopPanel formData={formData} issueTypeId={NODE_IDS.ISSUE_TYPE} detailedIssueId={NODE_IDS.DETAILED_ISSUE} purchaseInfoId={NODE_IDS.PURCHASE_INFO} getFinalNote={() => buildNoteText(formData)} cloudGenerate={sopCloudGenerateEnabled ? sopCloudGenerate : undefined} llmGenerate={llmParser.enabled ? llmParser.generate : null} llmStatus={llmParser.status} llmIsReady={llmParser.isReady} warmLlm={llmParser.load}/>),
            productContent: (<ProductLookupPanel robotModel={String(formData[NODE_IDS.DEEBOT_MODEL] ?? '')} issueDescription={String(formData[NODE_IDS.DETAILED_ISSUE] ?? '')} issueType={String(formData[NODE_IDS.ISSUE_TYPE] ?? '')}/>),
        }}>
            <FlowchartCanvas nodes={nodes} positions={positions} formData={formData} onFieldChange={handleFieldChange} activeNodeId={activeNodeId} onNodeFocus={handleNodeFocus} onNodeBlur={handleNodeBlur} onPositionChange={handlePositionChange} onHangUp={handleHangUp} hangUpLoading={hangUpRunning} autoFocusId={NODE_IDS.DETAILED_ISSUE} onLayoutReset={handleLayoutReset} parsedFields={parsedFields} hiddenNodes={hiddenNodesSet}/>
          </TicketPanelsContext.Provider>
        </main>
      </div>

      <OutputModal open={showOutput} onOpenChange={setShowOutput} noteText={noteText} onSaveToHistory={handleOutputClose}/>

      <TemplatePanel template={openTemplate} onClose={handleCloseTemplate} onInsertLine={handleInsertTemplateLine} resolutionText={typeof formData[NODE_IDS.RESOLUTION_SUMMARY] === 'string'
            ? formData[NODE_IDS.RESOLUTION_SUMMARY]
            : ''} onResolutionChange={handleResolutionChange}/>

      {extensionBridge.pendingPush && (<ExtensionPushConfirmPopup pending={extensionBridge.pendingPush} labels={EXT_PUSH_FIELD_LABELS} onApply={extensionBridge.acceptPendingPush} onDismiss={extensionBridge.dismissPendingPush}/>)}

      <Toaster position="bottom-right" theme={getThemeMeta(theme).toaster} toastOptions={{
            classNames: {
