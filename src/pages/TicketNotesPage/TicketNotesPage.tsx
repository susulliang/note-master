...
Additional information (if needed): ${getStr(NODE_IDS.ADDITIONAL_NOTES) || 'N/A'}`;
  ...
  //  plus base.fr / tiny.fr for French calls — the transcript stays in
  //  the original language and the LLM parse translates fields to
  //  English, so the ticket note stays in English.
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  //  Live EN/ZH whole-call TLDR — FR calls only (a multilingual .fr model
  //  active): the agent follows a French call they may not fully understand
  //  via the English summary (plus a Chinese one). Regenerated by the cloud
  //  LLM at 10s/30s/60s/90s… into the call; no key → no TLDR card.
  // ---------------------------------------------------------------------
  const callTldr = useCallTldr({
    isCapturing: call.isCapturing && isMultilingualModel(localWhisper.model),
