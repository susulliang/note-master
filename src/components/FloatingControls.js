 ...
 */
export default function FloatingControls({ theme, onCycleTheme, onReset, historyOpen, onToggleHistory, history, onDeleteHistory, onClearHistory, uiScale, onToggleUiScale, callSupported, callCapturing, onToggleCall, engine, parser, cloud, transcript, isTranscribing, gridboxVisibility, }) {
    const themeMeta = getThemeMeta(theme);
    const ThemeIcon = themeMeta.icon;
    const [engineOpen, setEngineOpen] = useState(false);
    const [boxesOpen, setBoxesOpen] = useState(false);
    // Docked corner (persisted) + transient free position while dragging
    const [corner, setCorner] = useScopedState('ecovacs_ticket_toolbar_corner', 'tr');
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            const x = Math.min(Math.max(8, e.clientX - d.grabOffsetX), Math.max(8, window.innerWidth - d.width - 8));
            const y = Math.min(Math.max(8, e.clientY - d.grabOffsetY), Math.max(8, window.innerHeight - d.height - 8));
    ...
    return (<>
      ...
      {historyOpen && (<div className="fixed inset-0 z-40" onClick={onToggleHistory} aria-hidden="true"/>)}

      <div ref={containerRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className={cn('fixed z-50 flex gap-2', atTop ? 'flex-col' : 'flex-col-reverse', atRight ? 'items-end' : 'items-start', !isDragging && CORNER_CLASSES[corner])} style={isDragging && dragPos
            ? {
        ...
        <div className={cn('flex flex-col items-center gap-1', !atTop && 'flex-col-reverse')}>
          <div className={cn('glass-panel flex items-center gap-1 rounded-full p-1 transition-all duration-500', dimmed
            ? // Idle: 20% opacity, frost almost gone
                'glass-dim opacity-20'
            : // Hovering: full frosted glass
                'opacity-100', isDragging && 'glass-active')}>
            <Button variant="ghost" size="icon" onClick={onToggleHistory} className={cn('relative size-8 rounded-full text-muted-foreground hover:text-foreground', historyOpen && 'bg-foreground/10 text-foreground')} aria-label="Toggle history" title="History">
              <History className="size-4"/>
              {history.length > 0 && (<span className="absolute -right-0.5 -top-0.5 flex size-4 items-center justify-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="ghost" size="icon" className="size-8 rounded-full text-muted-foreground hover:text-foreground" aria-label="Reset form" title="Reset">
            ...
            {gridboxVisibility && (<Button variant="ghost" size="icon" onClick={() => {
                setBoxesOpen((v) => !v);
                if (engineOpen)
                    setEngineOpen(false);
            }} className={cn('relative size-8 rounded-full text-muted-foreground hover:text-foreground', boxesOpen && 'bg-foreground/10 text-foreground')} aria-label="Toggle gridboxes" title="Gridboxes — show / hide Shipping address, Transcript, 24h tracker, SOP, SKU, Serial, Additional notes, Matching template">
                ...
                if (hiddenCount === 0)
                    return null;
                return (<span className="absolute -right-0.5 -top-0.5 flex size-4 items-center justify-center rounded-full bg-amber-500/90 text-[9px] font-bold text-black">
                      {hiddenCount}
                    </span>);
            })()}
              </Button>)}
            ...
            {callSupported && (<Button variant="ghost" size="icon" onClick={onToggleCall} className={cn('relative size-8 rounded-full text-muted-foreground hover:text-foreground', callCapturing &&
                'bg-destructive/15 text-destructive hover:bg-destructive/25 hover:text-destructive')} aria-label={callCapturing ? 'Stop call capture' : 'Capture CCP call audio'} title={callCapturing
                ...
                {callCapturing ? (<MicOff className="size-4"/>) : (<Mic className="size-4"/>)}
                {callCapturing && (<span className="absolute -right-0.5 -top-0.5 flex size-2.5">
            ...
            <Button variant="ghost" size="icon" onClick={onToggleUiScale} className={cn('size-8 rounded-full text-muted-foreground hover:text-foreground', uiScale === 'large' && 'bg-primary/15 text-primary')} aria-label="Toggle larger text" title={uiScale === 'large' ? 'Larger text: on — click to turn off' : 'Larger text: off — click to turn on'}>
              <Type className="size-4"/>
            </Button>

            {/* Engine settings — the gear opens the Whisper/LLM/downloads/
            resources/debug panel (like the History button opens history) */}
            {engine && parser && (<Button variant="ghost" size="icon" onClick={() => setEngineOpen((v) => !v)} className={cn('relative size-8 rounded-full text-muted-foreground hover:text-foreground', engineOpen && 'bg-foreground/10 text-foreground')} aria-label="Engine settings" title="Engine settings — Whisper & LLM models, downloads, resources, debug">
                <Settings className="size-4"/>
                {(engine.status === 'error' || parser.status === 'error') && (<span className="absolute -right-0.5 -top-0.5 flex size-2.5">
            ...
            <Button variant="ghost" onClick={onCycleTheme} className="h-8 gap-1.5 rounded-full px-2.5 text-muted-foreground hover:text-foreground" aria-label={`Switch theme (current: ${themeMeta.label})`} title={`Theme: ${themeMeta.label} — click to cycle`}>
          ...
          <div onMouseDown={handleDragStart} aria-label="Drag toolbar to a corner" title="Drag to move" className={cn('glass-chip flex h-5 w-16 cursor-grab items-center justify-center rounded-full transition-opacity duration-300 active:cursor-grabbing', handleVisible || isDragging ? 'opacity-100' : 'pointer-events-none opacity-0')}>
            <div className="h-1 w-10 rounded-full bg-foreground/45 shadow-[0_1px_3px_rgba(0,0,0,0.4)]"/>
          </div>
        </div>

        {historyOpen && (<HistoryPanel history={history} onDeleteHistory={onDeleteHistory} onClearHistory={onClearHistory} onClose={onToggleHistory}/>)}
        ...
        {boxesOpen && gridboxVisibility && (<>
            <div className="fixed inset-0 z-40" onClick={() => setBoxesOpen(false)} aria-hidden="true"/>
            <div className="relative z-50">
              <div className="glass-panel w-72 rounded-2xl p-3 text-[11px] shadow-2xl animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="mb-1.5 flex items-center justify-between px-0.5">
                  ...
                  <button type="button" className="flex size-6 items-center justify-center rounded-full text-muted-foreground transition hover:bg-foreground/10 hover:text-foreground" onClick={() => setBoxesOpen(false)} aria-label="Close gridboxes panel" title="Close">
                    ✕
                  </button>
                </div>
                <div className="mt-2 flex flex-col gap-0.5 rounded-xl border border-foreground/10 bg-foreground/[0.03] p-1.5">
                  {gridboxVisibility.toggles.map((t) => (<div key={t.id} className="group flex items-center gap-2 rounded-md px-1.5 py-1.5 transition hover:bg-foreground/[0.05]">
                      <div className="min-w-0 flex-1">
                        <div className={cn('truncate text-[11px]', t.visible ? 'text-foreground' : 'text-muted-foreground/70 line-through decoration-muted-foreground/50')}>
                          {t.label}
                        </div>
                      </div>
                      <Switch checked={t.visible} onCheckedChange={(next) => gridboxVisibility.onToggle(t.id, !!next)} aria-label={`Toggle ${t.label}`}/>
                    </div>))}
                </div>
        ...
        {engineOpen && engine && parser && (<>
            ...
            <div className="relative z-50">
              <EngineSettingsPanel engine={engine} parser={parser} cloud={cloud} transcript={transcript ?? []} isCapturing={callCapturing} isTranscribing={!!isTranscribing} onToggleCapture={onToggleCall} onClose={() => setEngineOpen(false)}/>
            </div>
          </>)}
      </div>
    </>);
