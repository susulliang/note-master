import { createContext, memo, useContext, useRef, useState, useCallback, useEffect, useMemo, } from 'react';
...
export const TicketPanelsContext = createContext(null);
...
function ComboboxField({ label, value, options, onChange, onFocus, onBlur, icon: Icon, width, }) {
    ...
    const filteredGroups = useMemo(() => {
        ...
        if (!query)
            return groups;
        return groups
            .map(([cat, items]) => [cat, items.filter((o) => o.toLowerCase().includes(query))])
    ...
    const visibleGroups = filteredGroups
        ...
        .filter((g) => g !== null && g[1].length > 0);
    const isCustomValue = value.trim().length > 0 && !options.includes(value);
    return (<div className="px-2.5 py-1.5">
      {label && (<div className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
      ...
      <div ref={wrapperRef} className="relative flex items-center">
        <Input value={value} onChange={(e) => {
            ...
            if (e.key === 'Escape')
                setOpen(false);
        }} placeholder="Type or select..." className="h-9 pr-8 text-sm"/>
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <button type="button" aria-label="Toggle options" className="absolute right-1 top-1/2 flex size-6 -translate-y-1/2 items-center justify-center rounded text-muted-foreground hover:bg-accent/10 hover:text-foreground" onMouseDown={(e) => {
          ...
          <PopoverContent align="start" sideOffset={4} style={{ width: Math.max(300, width ?? 300) }} className="glass-panel rounded-xl p-0" onInteractOutside={(e) => {
            ...
            <Command shouldFilter={false}>
              <CommandList className="max-h-[280px]">
                {filteredGroups.length === 0 ? (<CommandEmpty>
                    {value.trim()
                ? 'No match — your text will be kept as a custom value'
                : 'No options'}
                  </CommandEmpty>) : (visibleGroups.map(([cat, items]) => (<CommandGroup key={cat || 'all'} heading={cat || undefined}>
                      {items.map((opt) => {
                const sep = opt.indexOf('::');
                const display = sep > 0 ? opt.slice(sep + 2) : opt;
                return (<CommandItem key={opt} value={opt} onSelect={() => {
                        onChange(opt);
                        setOpen(false);
                    }} className="gap-2 text-sm">
                            <Check className={cn('size-3.5 shrink-0', value === opt ? 'opacity-100' : 'opacity-0')}/>
                            <span className="truncate">{display}</span>
                          </CommandItem>);
            })}
                    </CommandGroup>)))}
                {truncated && (<div className="px-2 py-1.5 text-center text-[11px] text-muted-foreground">
      ...
      {isCustomValue && (<div className="mt-1 text-[10px] uppercase tracking-wider text-accent/80">
          Custom value
        </div>)}
    </div>);
}
function FlowNodeComponent({ id, type, label, text, value, onChange, onFocus, onBlur, isActive, position, onDragStart, options = [], accent = 'default', inputType = 'text', width = 240, textareaRows = 2, autoFocus = false, icon: Icon, quickTexts, quickTextGroups, customQuickTexts, onAddQuickText, onRemoveQuickText, templateMatches, onOpenTemplate, zIndex, onHeightChange, parsedSource = null, enablePinBubble = false, panelContent, hangUpLoading = false, }) {
    ...
    const pinPressTimerRef = useRef(null);
     ...
     */
    const handleInsertQuickText = useCallback((quickText) => {
    ...
    const handleMouseDown = useCallback((e) => {
        ...
        const interactive = target.closest('input, textarea, button, select, [contenteditable="true"], [role="combobox"], [role="listbox"]');
    ...
    useEffect(() => {
        ...
        const raf = requestAnimationFrame(() => {
            const el = nodeRef.current?.querySelector('textarea, input');
            el?.focus();
        });
        return () => cancelAnimationFrame(raf);
    }, [autoFocus]);
    /** Shared quick-insert chip (optionally with a hover remove badge) */
    const renderQuickChip = (qt, onRemove) => (<span key={qt} className="group relative">
      <button type="button" onClick={() => handleInsertQuickText(qt)} title={`Insert: ${qt}`} className="glass-chip h-7 min-w-0 max-w-full truncate rounded-md px-2 text-[11px] font-semibold">
        {qt}
      </button>
      {onRemove && (<button type="button" onClick={onRemove} aria-label={`Remove quick text: ${qt}`} title={`Remove: ${qt}`} className="absolute -right-1 -top-1 flex size-3.5 items-center justify-center rounded-full border border-background bg-destructive text-destructive-foreground opacity-0 transition-opacity group-hover:opacity-100">
          <X className="size-2"/>
        </button>)}
    </span>);
    const renderContent = () => {
        if (type === 'start' || type === 'agent') {
            return (<div className="px-2.5 py-1.5">
          {label && (<div className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              {label}
            </div>)}
        ...
        if (type === 'hangup') {
            return (<div className="px-2.5 py-2">
          <button type="button" disabled={hangUpLoading} onClick={() => {
                    if (hangUpLoading)
                        return;
                    ...
                    onChange('hangup');
                }} className={cn('glass-btn glass-btn-destructive group flex min-h-12 w-full items-center justify-center gap-2 whitespace-nowrap rounded-lg px-3 text-sm font-semibold text-destructive-foreground transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', hangUpLoading && 'pointer-events-none opacity-80 ring-2 ring-destructive/50')}>
            {hangUpLoading ? (<Loader2 className="size-4 shrink-0 animate-spin"/>) : (<PhoneOff className="size-4 shrink-0 transition-transform duration-200 group-hover:rotate-12"/>)}
        ...
        if (type === 'templates') {
          ...
          <div className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            {Icon && <Icon className="size-3.5 text-accent/70"/>}
            {label}
            {matches.length > 0 && (<span className="rounded-full bg-accent/15 px-1.5 text-[9px] font-semibold text-accent">
                {matches.length}
              </span>)}
          </div>
          {matches.length > 0 && onOpenTemplate ? (<div className="flex flex-wrap gap-1">
              {matches.map((tpl) => (<button key={`${tpl.kind}-${tpl.file}`} type="button" onClick={() => onOpenTemplate(tpl)} title={`Open ${tpl.category}: ${tpl.name}`} className={cn('glass-chip h-7 min-w-0 max-w-full truncate rounded-md px-2 text-[11px] font-semibold', tpl.kind === 'amr' && 'glass-chip-accent')}>
                  <span className={cn('mr-1 rounded px-1 text-[8px] font-bold uppercase tracking-wider', tpl.kind === 'amr' && 'bg-accent/20 text-accent', tpl.kind === 'tbs' && 'bg-primary/20 text-primary', tpl.kind === 'err' && 'bg-destructive/25 text-destructive', tpl.kind === 'macro' &&
                            'bg-emerald-500/25 text-emerald-600 dark:text-emerald-300', tpl.kind === 'faq' && 'bg-warning/25 text-warning')}>
                    {tpl.kind === 'amr'
                            ? 'AMR'
                            : tpl.kind === 'tbs'
                                ? 'TBS'
            ...
            </div>) : (<p className="text-[11px] text-muted-foreground/80">
        ...
        if (type === 'select') {
            return (<ComboboxField label={label} value={typeof value === 'string' ? value : ''} options={options} onChange={onChange} onFocus={handleFocus} onBlur={onBlur} icon={Icon} width={width}/>);
        }
        if (type === 'dynamic-list') {
          ...
          <div className="space-y-1.5">
            {steps.map((step, idx) => (<div key={idx} className="flex items-center gap-1.5">
                ...
                <Input value={step} onChange={(e) => {
                        ...
                        onChange(newSteps);
                    }} onFocus={handleFocus} onBlur={onBlur} className="h-8 text-sm"/>
                <Button type="button" variant="ghost" size="icon" className="!absolute right-1 top-1 h-6 w-6 shrink-0 text-muted-foreground hover:text-destructive" onClick={() => {
            ...
            <Button type="button" variant="ghost" size="sm" className="w-full gap-1.5 text-xs text-muted-foreground hover:text-foreground" onClick={() => onChange([...steps, ''])}>
        ...
        {inputType === 'textarea' ? (<Textarea ref={textareaRef} data-field-id={id} value={strValue} onChange={(e) => onChange(e.target.value)} onFocus={handleFocus} onBlur={onBlur} rows={textareaRows} className="resize-none text-sm" placeholder="Type here..."/>) : (<div className="relative" onMouseDown={startPinPress} onMouseUp={cancelPinPress} onMouseLeave={cancelPinPress}>
            <Input type={inputType} data-field-id={id} value={strValue} onChange={(e) => onChange(e.target.value)} onFocus={handleFocus} onBlur={onBlur} className="h-9 text-sm" placeholder="Type here..."/>
            {/* HIDDEN PIN bubble — press-and-hold the field to reveal */}
            {showPin && (<div className="absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2">
        ...
        {quickTextGroups ? (
            ...
            <div className="relative mt-1.5 border-t border-border/30 pt-1.5" onMouseEnter={() => setQuickPanelOpen(true)} onMouseLeave={() => setQuickPanelOpen(false)}>
            <div className="mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              <span className="ml-auto flex items-center gap-0.5 text-[9px] normal-case tracking-normal text-muted-foreground/70">
                hover to expand
                <ChevronDown className={cn('size-3 transition-transform duration-300', quickPanelOpen && 'rotate-180')}/>
              </span>
            </div>
            {/* Collapsed preview */}
            <div className="flex flex-wrap gap-1">
              {previewItems.map((qt) => renderQuickChip(qt))}
              {hiddenCount > 0 && (<span className="flex h-7 items-center rounded-md border border-dashed border-foreground/20 px-2 text-[11px] text-muted-foreground/80">
                  +{hiddenCount} more
                </span>)}
            </div>
            {/* In-flow expansion — the node itself grows */}
            {quickPanelOpen && (<div className="custom-scrollbar mt-1.5 max-h-[320px] overflow-y-auto rounded-lg border border-foreground/10 bg-foreground/[0.03] p-2">
                {customQuickTexts && customQuickTexts.length > 0 && (<div className="mb-2 last:mb-0">
                    <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                      Custom
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {customQuickTexts.map((qt) => renderQuickChip(qt, () => onRemoveQuickText?.(qt)))}
                    </div>
                  </div>)}
                {quickTextGroups.map((g) => (<div key={g.label} className="mb-2 last:mb-0">
                    <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                      {g.label}
                    </div>
                    <div className="flex flex-wrap gap-1">
                ...
                {onAddQuickText && (<div className="mt-1 border-t border-border/30 pt-1.5">
                    {showAddQuickText ? (<Input value={newQuickText} onChange={(e) => setNewQuickText(e.target.value)} onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                            ...
                            }} placeholder="New quick text + Enter" autoFocus className="h-7 text-[11px]"/>) : (<button type="button" onClick={() => {
                                setNewQuickText('');
            ...
            <div className="flex flex-wrap gap-1">
              {quickTexts.map((qt) => renderQuickChip(qt, customQuickTexts?.includes(qt) && onRemoveQuickText
                    ? () => onRemoveQuickText(qt)
                    : undefined))}
              {onAddQuickText && (<button type="button" onClick={() => {
                        setShowAddQuickText((v) => !v);
                        if (!showAddQuickText) {
                            setNewQuickText('');
                        }
                    }} aria-label="Add quick text" title="Add quick text" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-dashed border-foreground/20 text-muted-foreground transition-colors hover:border-accent/60 hover:bg-accent/15 hover:text-accent">
                  <Plus className="size-3"/>
                </button>)}
            </div>
            {showAddQuickText && onAddQuickText && (<Input value={newQuickText} onChange={(e) => setNewQuickText(e.target.value)} onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                            e.preventDefault();
                            handleAddQuickTextSubmit();
                        }
                    ...
                    }} placeholder="New quick text + Enter" autoFocus className="mt-1.5 h-7 text-[11px]"/>)}
          </div>) : null}
      </div>);
    };
    const isPanelNode = type === 'transcript' ||
        type === 'ticketTracker' ||
        type === 'sop' ||
        type === 'productLookup';
    return (<div ref={nodeRef} style={{
            left: position.x,
            top: position.y,
            width,
            ...(zIndex !== undefined ? { zIndex } : {}),
        }} className={cn('absolute cursor-grab select-none rounded-xl transition-all duration-200 active:cursor-grabbing', '[&_button]:cursor-pointer [&_input]:cursor-text [&_textarea]:cursor-text [&_input]:select-text [&_textarea]:select-text', 
        ...
        parsedSource === 'llm' && !isPanelNode && 'glass-parsed', parsedSource && parsedSource !== 'llm' && !isPanelNode && 'glass-parsed-regex', 
        // Expanded (in-flow) quick-inserts: node grows over neighbours and
        // turns much frostier for readability
        quickPanelOpen && 'glass-expanded z-30')} onMouseDown={handleMouseDown}>
      ...
      {parsedSource && (<span className={cn('absolute right-2 top-2 z-10 rounded-full border px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider', parsedSource === 'llm'
                ...
                : parsedSource === 'paraphrase'
                    ...
                    : parsedSource === 'dom-ext'
                        ...
                        : 'border-sky-400/40 bg-sky-400/15 text-sky-600 dark:text-sky-300')} title={parsedSource === 'llm'
