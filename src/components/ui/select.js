...
function Select({ value, defaultValue, onValueChange, ...props }) {
    ...
    return (<SelectPrimitive.Root data-slot="select" value={mappedValue} defaultValue={mappedDefaultValue} onValueChange={handleChange} {...props}/>);
}
...
function SelectItem({ className, children, disabled, onClick, onMouseDown, onKeyDown, onKeyUp, onPointerDown, value, ...props }) {
    return (<SelectPrimitive.Item data-slot="select-item" className={cn("focus:bg-accent focus:text-accent-foreground data-[state=checked]:text-primary data-[state=checked]:[&_svg:not([class*='text-'])]:text-primary relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2", className)} value={value === "" ? EMPTY_SENTINEL : value} disabled={disabled} onClick={(event) => {
            if (disabled) {
                event.preventDefault();
                return;
            }
            onClick?.(event);
        }} onMouseDown={disabled ? undefined : onMouseDown} onKeyDown={disabled ? undefined : onKeyDown} onKeyUp={disabled ? undefined : onKeyUp} onPointerDown={(event) => {
            if (disabled) {
                event.preventDefault();
                return;
            }
