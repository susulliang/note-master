 ...
 */
export function resolveSopImageSrc(rawSrc: string): string {
...
function splitPipeRow(raw: string): string[] {
...
function isPlausibleTableRow(line: string): boolean {
...
function tryUnfoldCrushedTable(line: string): string[] | null {
...
export interface RenderBodyMarkdownOptions {
  onImageClick?: (resolvedSrc: string, altText: string) => void;
}
...
export function renderBodyMarkdown(
  lines: string[],
  opts: RenderBodyMarkdownOptions = {}
): React.ReactElement[] {
  ...
  for (const raw of lines) {
    ...
    if (unfolded) {
      for (const row of unfolded) normalizedLines.push(row);
    } else {
  ...
  const inline = (s: string): (string | React.ReactNode)[] => {
    type Token =
      | { kind: 'text'; val: string }
      | { kind: 'img'; alt: string; rawSrc: string }
      | { kind: 'link'; label: string; href: string }
      | { kind: 'code'; val: string }
      | { kind: 'strong'; val: string; marker: '**' | '__' }
      | { kind: 'em'; val: string; marker: '*' | '_' }
      | { kind: 'br' };
    const tokens: Token[] = [];
    ...
    for (const t of tokens) {
      switch (t.kind) {
        ...
        case 'img': {
          ...
          try { baseFn = decodeURIComponent(baseFn); } catch { /* keep raw */ }
          out.push(
            <span
              key={`img-${k++}`}
            ...
            >
              <button
                ...
                title={`Click to open full-size image: ${baseFn}${alt && alt !== baseFn ? ` / ${alt}` : ''}`}
              >
                {resolved ? (
                  <img
                    src={resolved}
                    ...
                    onError={(e) => {
                      ...
                      const sib = (e.currentTarget as HTMLImageElement).parentElement?.querySelector<HTMLElement>(
              ...
              <span className="flex items-center gap-1 truncate px-0.5">
                ...
                {alt && alt !== baseFn && (
                  <span className="ml-auto max-w-[55%] truncate text-muted-foreground/70" title={alt}>— {alt}</span>
                )}
              </span>
            </span>
          );
        ...
        case 'code':
          out.push(
            <code
              key={`c-${k++}`}
              className="rounded bg-foreground/10 px-1 py-0.5 text-[11px] font-mono text-foreground/90"
            >
              {t.val}
            </code>
          );
        ...
        case 'strong':
          out.push(
            <strong key={`b-${k++}`} className="text-foreground font-semibold">
              {inline(t.val)}
            </strong>
          );
          break;
        case 'em':
          out.push(
            <em key={`i-${k++}`} className="text-foreground/95 italic">
              {inline(t.val)}
            </em>
          );
          break;
  ...
  while (i < L.length) {
    ...
    // Headings (#, ##, ###)
    {
      ...
      if (h) {
        ...
        blocks.push(
          <div
            key={`h-${keyRef.n++}`}
            className={cn('mt-3 mb-1 text-foreground', sizeClass)}
          >
            {inline(h[2])}
          </div>
        );
    ...
    if (/^>\s?/.test(raw)) {
      ...
      blocks.push(
        <blockquote
          key={`bq-${keyRef.n++}`}
          className="my-1.5 border-l-2 border-accent/60 bg-accent/5 pl-3 pr-2 py-1.5 text-[13px] text-foreground/90 rounded-r-md"
        >
          {buf.map((l, idx) => (
    ...
    if (isPlausibleTableRow(raw)) {
      ...
      if (isTableSeparator(next)) {
        ...
        while (i < L.length) {
          const r = L[i] ?? '';
          if (r.trim() === '') break;
          if (!isPlausibleTableRow(r)) break;
        ...
        blocks.push(
          <div key={`tbl-${keyRef.n++}`} className="my-2 overflow-x-auto">
            <table className="w-full min-w-[420px] border-collapse border border-border bg-card/30 text-[12px] text-foreground/95">
                  ...
                  {headerCells.map((cell, c) => {
                    ...
                    return (
                      <th
                        key={c}
                        className={cn(
                          'border border-border bg-primary/20 px-3 py-1.5 text-[12px] font-semibold text-foreground',
                          alignClass(a)
                        )}
                      >
                        {inline(cell)}
              ...
              <tbody>
                {bodyRows.map((row, ri) => (
                  <tr key={ri} className={cn(ri % 2 === 1 ? 'bg-foreground/[0.04]' : undefined)}>
    ...
    if (/^\s*[-*+]\s+/.test(raw)) {
      ...
      const renderRows = (from: number, parentIndent: number): [React.ReactElement[], number] => {
        ...
        while (j < rows.length) {
          ...
          if (r.indent > parentIndent) {
            ...
            if (prev) {
              const prevProps = prev.props as { children?: React.ReactNode };
              const cloned: React.ReactElement = React.cloneElement<any>(prev, {
                children: (
                  <>
                    {prevProps.children}
      ...
      blocks.push(
        <ul
          key={`ul-${keyRef.n++}`}
          className="my-1 list-disc space-y-0.5 pl-5 text-[13px] text-foreground/90 marker:text-muted-foreground/70"
        >
          {rootItems}
        </ul>
      );
    ...
    if (/^\s*\d+[.)]\s+/.test(raw)) {
      ...
      blocks.push(
        <ol
          key={`ol-${keyRef.n++}`}
          className="my-1 list-decimal space-y-0.5 pl-5 text-[13px] text-foreground/90 marker:text-muted-foreground/70"
        >
          {items.map((it, idx) => (
    ...
    blocks.push(
      <p
        key={`p-${keyRef.n++}`}
        className="my-1 text-[13px] leading-relaxed text-foreground/90 whitespace-pre-wrap"
      >
        {inline(pLines.join(' '))}
      </p>
    );
