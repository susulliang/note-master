 ...
 */
function estimateChipRows(texts, contentWidth) {
    let rows = 1;
    let line = 0;
...
function estimateNodeHeight(node, value) {
 ...
 */
function computeDefaultLayout(nodes, canvasWidth, heightOf, hiddenNodes) {
    ...
    const leftColNodesVisible = LEFT_COL_IDS
    ...
    const isWide = Boolean(anyLeftColVisible &&
    ...
    if (isWide)
        mainAvailLeft = flowPaneCandidateLeft;
    ...
    const leftColSetWide = isWide ? new Set(LEFT_COL_IDS) : new Set();
    let y = yCursor;
    for (const row of NODE_LAYOUT_ROWS) {
        ...
        for (const lineNodes of lines) {
            ...
            if (gaps > 0) {
                colGap = Math.min(MAX_COL_GAP, Math.max(MIN_COL_GAP, (flowAvailWidth - sumWidth) / gaps));
            }
 ...
 */
const FlowchartCanvas = memo(function FlowchartCanvas({ nodes, positions, formData, onFieldChange, activeNodeId, onNodeFocus, onNodeBlur, onPositionChange, onHangUp, hangUpLoading = false, autoFocusId, onLayoutReset, parsedFields, hiddenNodes, }) {
    ...
    const heightOf = useCallback((n) => measuredHeights[n.id] ?? estimateNodeHeight(n, formData[n.id] ?? ''), [measuredHeights, formData]);
    // Responsive default layout; stored positions (user drags) take precedence
    const defaultLayout = useMemo(() => computeDefaultLayout(effectiveNodes, containerWidth, heightOf, hiddenNodes), [effectiveNodes, containerWidth, heightOf, hiddenNodes]);
    ...
    const canvasWidth = useMemo(() => {
        ...
        for (const n of effectiveNodes) {
            ...
            if (p)
                w = Math.max(w, p.x + (n.width ?? 240) + 80);
        }
        return w;
    ...
    const canvasHeight = useMemo(() => {
        ...
        for (const n of effectiveNodes) {
            const p = effectivePositions[n.id];
            if (p)
                h = Math.max(h, p.y + heightOf(n) + 80);
        }
        return h;
    ...
    const renderConnections = () => {
        return NODE_CONNECTIONS.map((conn, idx) => {
            ...
            if (hiddenNodes?.has(conn.from) || hiddenNodes?.has(conn.to))
                return null;
            ...
            return (<path key={idx} d={path} fill="none" stroke={isActive ? 'var(--primary)' : 'var(--border)'} strokeWidth={isActive ? 2 : 1.5} strokeDasharray={isActive ? 'none' : '4 4'} className="transition-all duration-300" opacity={isActive ? 0.9 : 0.55}/>);
        });
    };
    return (<div ref={canvasRef} className="custom-scrollbar relative h-full w-full overflow-auto bg-[radial-gradient(circle_at_1px_1px,color-mix(in_oklab,var(--foreground)_9%,transparent)_1px,transparent_0)] [background-size:24px_24px]">
      <div className="relative" style={{ width: canvasWidth, height: canvasHeight }}>
        ...
        {/* Nodes layer */}
        {effectiveNodes.map((node) => (<FlowNode key={node.id} id={node.id} type={node.type} label={node.label} text={node.text} value={formData[node.id] ?? (node.type === 'dynamic-list' ? [] : '')} onChange={(val, discrete) => {
                if (node.type === 'hangup') {
                    onHangUp();
                }
                else {
            ...
            }} onFocus={onNodeFocus} onBlur={onNodeBlur} isActive={activeNodeId === node.id} position={effectivePositions[node.id] ?? { x: CANVAS_MARGIN, y: CANVAS_MARGIN }} 
        // Chips-bearing boxes sit above neighbours; the node being
        // dragged floats above everything
        zIndex={dragging?.id === node.id
                ? 50
                : node.type === 'templates' || node.quickTexts
                    ? 10
                    : undefined} onDragStart={handleDragStart} onHeightChange={handleNodeHeightChange} options={node.options} accent={node.accent} inputType={node.inputType} width={node.width} textareaRows={node.textareaRows} autoFocus={node.id === autoFocusId} icon={node.icon} quickTexts={node.quickTexts} quickTextGroups={node.quickTextGroups} customQuickTexts={node.customQuickTexts} onAddQuickText={node.onAddQuickText} onRemoveQuickText={node.onRemoveQuickText} templateMatches={node.templateMatches} onOpenTemplate={node.onOpenTemplate} parsedSource={parsedFields?.[node.id] ?? null} enablePinBubble={node.pinFromValue} panelContent={node.panelContent} hangUpLoading={node.type === 'hangup' ? hangUpLoading : undefined}/>))}
      </div>
    </div>);
});
