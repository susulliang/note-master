...
import { Package, Search, Sparkles, ChevronRight, AlertTriangle, X, Link2, BookOpen, Cpu, Tag, Info, Lightbulb, HelpCircle, Languages, } from 'lucide-react';
import { findModels, freeSearch, getProductIndex, searchFaqs, } from '@/utils/productData';
 ...
 */
export default function ProductLookupPanel({ robotModel, issueDescription = '', issueType = '', }) {
    ...
    const errorCodeHits = useMemo(() => {
        ...
        for (const t of q.toLowerCase().split(/\W+/))
            if (t)
                qTokens.add(t);
    ...
    const sellingHits = useMemo(() => {
        if (!selectedModelName && !manualQueryDebounced)
            return index.sellingPoints.slice(0, 3);
    ...
    const scientistHits = useMemo(() => {
        if (!selectedModelName && !manualQueryDebounced)
            return index.scientistCodes.slice(0, 6);
        const q = (selectedModelName || '') + ' ' + manualQueryDebounced;
        const qTokens = new Set();
        for (const t of q.toLowerCase().split(/\W+/))
            if (t)
    ...
    // --- Free search hits -------------------------------------------------------
    const freeHits = useMemo(() => freeSearch(index, manualQueryDebounced || `${robotModel} ${issueType} ${issueDescription}`, 6), [index, manualQueryDebounced, robotModel, issueType, issueDescription]);
    // --- FAQ search hits --------------------------------------------------------
    ...
    useEffect(() => {
        if (errorCodeHits.length > 0)
            setActiveTab((prev) => (prev === 'specs' ? 'errors' : prev));
    ...
    const tabs = TABS_META.map((t) => ({ ...t, count: counts[t.kind] }));
    const handlePickSuggestion = useCallback((name) => {
        setPinnedModel(name);
        setActiveTab('specs');
    }, []);
    return (<div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-border bg-card/40 px-2 py-1.5 backdrop-blur-sm">
        ...
        <Input value={manualQuery} onChange={(e) => {
            setManualQuery(e.target.value);
            setPinnedModel(null);
        }} placeholder="Search model, spec, error code (E601)…" className="!h-7 border-0 !bg-transparent px-0 py-0 text-xs font-semibold placeholder:text-muted-foreground/60 focus-visible:ring-0 focus-visible:ring-offset-0"/>
        {pinnedModel && (<button type="button" onClick={() => setPinnedModel(null)} className="flex shrink-0 items-center gap-1 rounded-md bg-primary/15 px-2 py-0.5 text-[10px] font-bold text-primary transition-colors hover:bg-primary/25" title="Release pinned model → auto-follow DEEBOT model dropdown">
            <Link2 className="size-3"/>
            {pinnedModel}
            <X className="size-3 opacity-70 hover:opacity-100"/>
          </button>)}
        {!pinnedModel && robotModel && (<div className="flex shrink-0 items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary/90">
      ...
      {modelHits.length > 0 && (<div className="flex flex-wrap items-center gap-1 rounded-lg border border-foreground/5 bg-foreground/[0.02] px-2 py-1.5">
          ...
          {modelHits.slice(0, 8).map((m) => {
                ...
                return (<button key={m.origin + m.name} type="button" onClick={() => m.errorCode ? setActiveTab('errors') : handlePickSuggestion(m.name)} className={cn('glass-chip inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-bold transition-colors', isSelected && !m.errorCode
                        ? '!bg-primary/25 !text-primary ring-1 ring-primary/40'
                        : m.errorCode
                            ? 'bg-red-500/10 text-red-400 hover:bg-red-500/20'
                            : 'hover:bg-foreground/10')} title={m.origin}>
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-border pb-1.5">
        {tabs.map((t) => {
            ...
            return (<button key={t.kind} type="button" onClick={() => setActiveTab(t.kind)} className={cn('group inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold transition-colors', active
                    ? 'bg-primary/15 text-primary ring-1 ring-primary/30'
                    : 'text-muted-foreground hover:bg-foreground/5 hover:text-foreground')}>
              <Icon className={cn('size-3.5', active ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground')}/>
              {t.label}
              <span className={cn('rounded px-1 text-[9px] font-bold', active ? 'bg-primary/25 text-primary' : 'bg-foreground/10 text-muted-foreground')}>
                {t.count}
              </span>
            </button>);
        })}
...
function SpecsTab({ sections, modelName, }) {
    if (sections.length === 0) {
        return (<EmptyState icon={Package} text={modelName
    ...
    for (const s of sections) {
        if (!bySheet.has(s.sheetTitle))
            bySheet.set(s.sheetTitle, []);
        bySheet.get(s.sheetTitle).push(s);
    }
    return (<div className="space-y-3">
      {Array.from(bySheet.entries()).map(([sheetTitle, secs]) => (<div key={sheetTitle}>
          <div className="mb-1 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-primary/80">
            <BookOpen className="size-3"/>
            {sheetTitle}
          </div>
          {secs.map((s) => (<div key={s.section} className="mb-2 overflow-hidden rounded-md border border-border/60">
              ...
              <div className="divide-y divide-border/40">
                {s.rows.map((r) => (<div key={r.spec + r.value.slice(0, 10)} className="grid grid-cols-[40%_1fr] gap-2 px-2 py-1.5 hover:bg-foreground/[0.03]">
...
function ErrorCodesTab({ rows }) {
    ...
    return (<div className="space-y-2">
      {rows.map((r) => (<div key={r.code} className="overflow-hidden rounded-md border border-red-500/20 bg-red-500/[0.04]">
          ...
          <div className="space-y-1.5 px-2 py-1.5">
            {r.meaning && (<div className="grid grid-cols-[52px_1fr] gap-2">
            ...
            {r.solution && (<div className="grid grid-cols-[52px_1fr] gap-2">
...
function FaqBadge({ className }) {
    return (<span className={cn('inline-flex items-center gap-1 rounded bg-warning/25 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-warning', className)}>
      <HelpCircle className="size-2.5"/>
      FAQ
    </span>);
}
function FaqTab({ hits }) {
    const [openId, setOpenId] = useState(null);
    if (hits.length === 0) {
        return (<EmptyState icon={HelpCircle} text="No FAQ matches yet. Select a model or type a question keyword (e.g. 噪音 / wifi / red light blinking)."/>);
    }
    return (<div className="space-y-2">
      {hits.map((f) => {
            const isOpen = openId === f.id;
            return (<div key={f.id} className={cn('overflow-hidden rounded-md border transition-all', 'border-warning/20 bg-warning/[0.03] hover:bg-warning/[0.05]')}>
            <button type="button" onClick={() => setOpenId(isOpen ? null : f.id)} className="flex w-full items-start gap-2 border-b border-warning/15 bg-warning/10 px-2 py-1 text-left">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-1 text-[9px] text-muted-foreground">
                  {f.category && (<span className="rounded bg-foreground/10 px-1 font-bold uppercase tracking-wider">{f.category}</span>)}
                  {f.model && f.modelSlug && f.model !== f.modelSlug && (<span className="truncate rounded bg-foreground/10 px-1">{f.model}</span>)}
                  {f.lang && (<span className="inline-flex items-center gap-0.5 rounded bg-foreground/10 px-1">
                      <Languages className="size-2"/> {f.lang}
                    </span>)}
                  {f.modelMatchScore > 0 && (<span className="rounded bg-success/15 px-1 font-bold text-success/80">
            ...
            {isOpen && (<div className="space-y-2 px-2 py-1.5">
                {f.question && f.question !== f.title && (<div>
                ...
                {f.sourceSheet && (<div className="text-[9px] text-muted-foreground/70">
...
function SellingTab({ rows, }) {
    if (rows.length === 0) {
        return <EmptyState icon={Sparkles} text="No selling-point hits. Pick a model or search a feature keyword."/>;
    }
    return (<div className="space-y-2">
      {rows.map((r, i) => (<div key={r.model + i} className="overflow-hidden rounded-md border border-primary/20 bg-primary/[0.03]">
          <div className="flex items-center gap-2 border-b border-primary/15 bg-primary/10 px-2 py-1">
            ...
            {(r.series || r.subSeries) && (<span className="ml-auto truncate text-[10px] text-primary/80">
...
function ScientistTab({ rows, }) {
    ...
    return (<div className="overflow-hidden rounded-md border border-border/60">
      ...
      <div className="divide-y divide-border/40">
        {rows.map((r, i) => (<div key={r.model + i} className="grid grid-cols-[1fr_1fr_1fr] gap-1 px-2 py-1.5 text-[11px] font-semibold hover:bg-foreground/[0.03]">
...
function FreeTab({ hits }) {
    ...
    return (<div className="space-y-2">
      {hits.map((h, i) => (<div key={h.sheetId + h.title + i} className="overflow-hidden rounded-md border border-border/60">
          ...
          <div className="px-2 py-1.5">
            ...
            <div className="whitespace-pre-wrap text-[11px] leading-relaxed text-foreground/80">
              {renderBodyMarkdown(h.body.split(/\r?\n/), {
                onImageClick: (src, alt) => setLightbox({ src: resolveSopImageSrc(src), alt }),
            })}
            </div>
          </div>
        </div>))}
      {lightbox && (<div role="dialog" className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur" onClick={() => setLightbox(null)}>
          <img src={lightbox.src} alt={lightbox.alt} className="max-h-[90vh] max-w-[90vw] rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} onError={(e) => {
