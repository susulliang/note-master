...
const KEY_FIELD_KEYS = ['contactNumber', 'customerName', 'deebotModel', 'serialNumber'];
...
export function useCcpExtensionBridge({ onApply, source = 'dom-ext', }) {
    ...
    const buildMapped = useCallback((fields) => {
        const mapped = {};
        const extras = [];
        for (const [k, raw] of Object.entries(fields || {})) {
            ...
            if (nodeId) {
                ...
                if (nice)
                    extras.push({ key: k, value: `${nice}: ${value}` });
            }
        }
        if (extras.length > 0) {
    ...
    const pickKeyFields = useCallback((fields) => {
        const out = {};
    ...
    const queuePendingPush = useCallback((rawFields, state, pushedAt) => {
    ...
    const applyMerged = useCallback((fields, state, _fromHello) => {
        if (!fields)
            return;
    ...
    const sendRequest = useCallback(async (request) => {
        ...
        const id = extIdRef.current || DEFAULT_EXT_ID_CANDIDATE;
        if (id && typeof globalThis.chrome?.runtime?.sendMessage === 'function') {
            try {
                return await new Promise((resolve, reject) => {
                    globalThis.chrome.runtime.sendMessage(id, request, (reply) => {
    ...
    const scrapeAll = useCallback(async () => {
        try {
            ...
            if (r?.ok) {
                const merged = {};
                for (const k of ['ccp', 'sf']) {
                    const d = r[k]?.payload?.data;
                    if (!d)
                        continue;
    ...
    const getSnapshot = useCallback(async () => {
        try {
            ...
            if (r?.ok)
                return { ok: true, merged: r.merged, state: r.state };
