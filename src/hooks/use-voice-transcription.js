...
import { extractFields, FIELD_PATTERNS, matchCanonicalModel, canonicalIssueType, classifyIssueType, summarizeIssueType, } from '@/lib/field-extraction';
...
export { extractFields, FIELD_PATTERNS, matchCanonicalModel, canonicalIssueType, classifyIssueType, summarizeIssueType, };
...
const SPEECH_ERROR_MESSAGES = {
    'not-allowed': 'Microphone permission denied — click the lock icon in the address bar, set Microphone to Allow, then reload.',
    'service-not-allowed': 'Microphone blocked by browser policy — allow mic access for this site and reload.',
    'audio-capture': 'No microphone audio — check the default input device (headset?) and that no other app is holding the mic.',
    network: 'Speech service unreachable (network error) — check your connection, it will keep retrying.',
 ...
 */
export function useVoiceTranscription(onAutoFill) {
    ...
    const startMeter = useCallback(async () => {
        try {
            ...
            if (name === 'NotAllowedError' || name === 'SecurityError') {
                setError('Microphone permission denied — click the lock icon in the address bar, set Microphone to Allow, then reload.');
            }
