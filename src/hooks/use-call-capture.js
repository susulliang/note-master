...
import { ACCUMULATING_FIELD_IDS, extractFields, isAsrArtifact, } from '@/lib/field-extraction';
...
const LLM_IDLE_DEBOUNCE_MS = 1500;
...
const LLM_MIN_INTERVAL_MS = 20000;
...
function readableCaptureError(err) {
    const name = err?.name ?? '';
 ...
 */
export function useCallCapture(onAutoFill, transcriber, llmParser, cloudParser) {
    ...
    const llmIdleTimerRef = useRef(null);
    ...
    const llmTickRef = useRef(() => undefined);
    ...
    const runParaphraseRef = useRef(() => undefined);
    ...
    const runParaphrase = useCallback(async () => {
        ...
        if (!parser || paraphraseRunningRef.current || llmRunningRef.current)
            return;
        const input = {};
        for (const key of ['issueDescription', 'resolutionSummary']) {
        ...
        try {
            ...
            for (const field of fields) {
                ...
                if (llmConfirmedRef.current.has(field.fieldId))
                    continue;
                paraphrasedFromRef.current[field.fieldId] = input[field.fieldId];
            ...
            const stillPending = paraphrasePendingRef.current.issueDescription ??
    ...
    const runRegexExtraction = useCallback(() => {
        const fields = extractFields(entriesRef.current).filter((f) => !llmConfirmedRef.current.has(f.fieldId));
        // Suggestions: regex finds under the LLM's authoritative reading
        setSuggestions(() => {
            const map = new Map();
            for (const f of fields)
                map.set(f.fieldId, f);
            for (const f of llmSuggestionsRef.current.values())
                map.set(f.fieldId, f);
            return [...map.values()];
        });
        ...
        for (const field of fields) {
            ...
            if (pushed === undefined) {
            ...
            else if (ACCUMULATING_FIELD_IDS.has(field.fieldId) &&
                field.value !== pushed) {
                // The accumulating extraction GREW as the call went on
                regexFilledRef.current.set(field.fieldId, field.value);
                const paraphrasable = field.fieldId === 'issueDescription' || field.fieldId === 'resolutionSummary';
                if (paraphrasable && canParaphrase) {
                    ...
                    // flash in the form
                    paraphrasePendingRef.current[field.fieldId] = field.value;
                    armParaphrase();
                }
                else {
   ...
   */
    ...
    function mergeAccumulated(fieldId, current, next) {
        const cur = (current ?? '').trim();
        if (!cur)
            return next;
        if (!CUMULATIVE_FIELD_IDS.has(fieldId))
            return next;
        if (next.trim() === cur)
            return next;
        const splitClauses = (v) => v
            .split(/\s*(?:;|->)\s*/)
            .map((c) => c.trim())
            .filter(Boolean);
        const curClauses = splitClauses(cur);
        const nextClauses = splitClauses(next);
        if (curClauses.length === 0 || nextClauses.length === 0)
            return next;
        ...
        const nextHasNew = nextClauses.some((n) => !curClauses.some((c) => lower(c).includes(lower(n)) || lower(n).includes(lower(c))));
        if (!nextHasNew)
        ...
        for (const n of nextClauses) {
            const dup = merged.some((c) => lower(c).includes(lower(n)) || lower(n).includes(lower(c)));
            if (!dup)
                merged.push(n);
        }
    ...
    const applyLlmFields = useCallback((fields) => {
        if (fields.length === 0)
            return;
        const applied = [];
        for (const field of fields) {
    ...
    const armIdleParse = useCallback((delayMs = LLM_IDLE_DEBOUNCE_MS) => {
     ...
     *    ticket.
     */
    const buildPriorValues = useCallback(() => {
    ...
    const runLlmParse = useCallback(async () => {
        const parser = llmParserRef.current;
        if (!parser || llmRunningRef.current)
            return;
        ...
        try {
            ...
            const fields = await parser.parse(entriesRef.current, PARSEABLE_FIELD_IDS, buildPriorValues());
     ...
     */
    const runCloudParse = useCallback(async () => {
        const cloud = cloudParserRef.current;
        if (!cloud || cloudRunningRef.current)
            return;
    ...
    const llmTick = useCallback(() => {
        if (llmRunningRef.current ||
            ...
            return;
        if (!llmParserRef.current)
            return;
        const textLen = entriesRef.current.reduce((n, e) => n + e.text.length, 0);
        if (entriesRef.current.length === 0 || textLen < LLM_MIN_TRANSCRIPT_CHARS)
            return;
    ...
    // -----------------------------------------------------------------
    const postSegment = useCallback(async (blob, speaker) => {
        ...
        try {
            ...
            setError(`Local transcription failed: ${err.message}`);
        }
        finally {
    ...
    const enqueueSegment = useCallback((blob, speaker) => {
     ...
     */
    const flushComplete = useCallback((throughSeq) => {
        while (nextFlushSeqRef.current < throughSeq) {
            ...
            if (!bucket)
                continue; // empty window (silence on both channels)
            if (bucket.customer)
                enqueueSegment(bucket.customer, 'customer');
            if (bucket.agent)
                enqueueSegment(bucket.agent, 'agent');
        }
    }, [enqueueSegment]);
     ...
     */
    const handleSegmentData = useCallback((speaker, event) => {
    ...
    const start = useCallback(async () => {
        ...
        if (display.getAudioTracks().length === 0) {
            ...
            setError('No tab audio in that share — click "Capture call" again, choose the CCP tab, and tick "Also share tab audio".');
            shouldCaptureRef.current = false;
            return;
        }
    ...
    const finalize = useCallback(async () => {
        ...
        while ((pendingCountRef.current > 0 || transcribingRef.current) &&
            Date.now() < deadline) {
            await sleep(150);
        }
