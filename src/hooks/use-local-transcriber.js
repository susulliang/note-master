...
import { readModelPref, writeModelPref, readDtypePref, writeDtypePref, } from '@/lib/whisper-models';
/**
...
export function useLocalTranscriber() {
    ...
    const pendingTranscribesRef = useRef(new Map());
    ...
    const switchModel = useCallback((target) => {
        if (target === modelRef.current && statusRef.current === 'ready')
            return;
        setModel(target);
    ...
    const isSupported = typeof Worker !== 'undefined' && typeof WebAssembly !== 'undefined' && typeof fetch === 'function';
    return {
        isSupported,
        model,
        status,
