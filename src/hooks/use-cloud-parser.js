...
import { parseWithDeepseek, getEffectiveApiKey, isDeepSeekDefault, readDeepseekApiKey, writeDeepseekApiKey, } from '@/lib/cloud-parser';
/**
...
export function useCloudParser() {
     ...
     */
    const parse = useCallback(async (entries, prior) => {
        ...
        try {
            ...
            localKey, entries, prior, (pct) => setProgress(pct));
            if (error) {
                setError(error);
                return [];
            }
