...
export const SHEET_RECORDS = buildSheetRecords();
/** Convenience helper kept for backwards compatibility / tests. */
export const PRODUCT_SHEETS_RAW = Object.fromEntries(SHEET_RECORDS.map((s) => [s.sheetKey, s.raw]));
function pickByPrefix(prefix) {
    return SHEET_RECORDS.find((s) => s.numericPrefix === prefix)?.raw;
}
const sheet01 = pickByPrefix(1) ?? '';
const sheet02 = pickByPrefix(2) ?? '';
const sheet03 = pickByPrefix(3) ?? '';
const sheet04 = pickByPrefix(4) ?? '';
const sheet05 = pickByPrefix(5) ?? '';
const sheet06 = pickByPrefix(6) ?? '';
const sheet07 = pickByPrefix(7) ?? '';
const sheet08 = pickByPrefix(8) ?? '';
const sheet09 = pickByPrefix(9) ?? '';
/* -------------------------------------------------------------------------- */
/*                               Token Utilities                              */
/* -------------------------------------------------------------------------- */
/**
...
export function textTokenOverlap(haystack, needle) {
    if (!needle.trim())
        return 0;
...
function parseGfmTable(block) {
    ...
    if (lines.length < 3)
        return null; // header + separator + 1 row minimum
    const sep = lines[1];
    if (!/^[\s|:\-—–]+$/.test(sep))
        return null;
    ...
    for (let i = 2; i < lines.length; i++) {
        ...
        if (row.every((c) => !c || c === '-' || c === '—'))
            continue;
        rows.push(row);
    }
    return { headers, rows };
}
function parseSheetTitle(md) {
 ...
 */
function shortenModelHeader(colHeader) {
...
export function parseComparisonSheet(sheetId, md) {
    ...
    let sheetModelLabels = null;
    const processTable = (sectionName, body) => {
        ...
        if (!sheetHeaders || !sheetModelLabels)
            return;
        const sectionMap = {};
        for (const row of parsed.rows) {
            const specName = row[0] ?? '';
            if (!specName || /^col-\d+$/.test(specName))
                continue;
            for (let i = 1; i < Math.min(sheetHeaders.length, row.length); i++) {
                const modelLabel = sheetModelLabels[i];
                if (/^col-\d+$/.test(modelLabel))
                    continue;
                const value = (row[i] ?? '').trim();
                if (!value || value === '-' || value === '—' || value === '✘')
                    continue;
                if (!sectionMap[modelLabel])
                    sectionMap[modelLabel] = {};
                sectionMap[modelLabel][specName] = value;
                if (!flatSpecs[modelLabel])
                    flatSpecs[modelLabel] = [];
    ...
    if (sectionBlocks.length > 0) {
        for (const block of sectionBlocks) {
            ...
            if (tableMatch)
                processTable(sectionName, tableMatch[2]);
        }
    }
    else {
        ...
        if (preambleEnd !== -1)
            processTable('Specifications', md.slice(preambleEnd));
    }
...
export function parseGoatErrorCodes(md) {
...
/* -------------------------------------------------------------------------- */
/** @internal exposed to test harness */
export function parseScientistCodes(md) {
    const tableMatch = md.match(/(^|\n)(\|[\s\S]*)$/);
    if (!tableMatch)
        return [];
    const parsed = parseGfmTable(tableMatch[2]);
...
/** @internal exposed to test harness */
export function parseSellingPoints(md) {
    const tableMatch = md.match(/(^|\n)(\|[\s\S]*)$/);
    if (!tableMatch)
        return [];
    const parsed = parseGfmTable(tableMatch[2]);
...
/** @internal exposed to test harness */
export function parseFreeTextSheet(sheetId, md) {
...
export function parseFaqMd(sheetKey, md) {
...
function extractSection(body, headingRe) {
    ...
    for (let i = startIdx; i < lines.length; i++) {
        ...
        if (i !== startIdx && /^#\s+\S/.test(lines[i].trim()))
            break;
...
export function getProductIndex() {
    ...
    for (const rec of SHEET_RECORDS) {
        ...
        if (rec.numericPrefix < 10)
            continue;
        const heading = rec.raw.match(/^##\s+(.+)$/m)?.[1]?.trim() ||
        ...
        if (parsed.models.length === 0 && Object.keys(parsed.sections).length === 0)
            continue;
        gbuComparisons.push(parsed);
    }
    const comparisons = [deebot, goat, winbot, techSpecs, ...gbuComparisons];
    const allModels = [];
    const seen = new Set();
    const push = (name, tokens, origin) => {
    ...
    for (const c of comparisons) {
        for (const m of c.models)
            push(m, c.modelTokens.get(m) ?? normalizeTokens(m), c.sheetTitle);
    }
    for (const s of sellingPoints)
        push(s.model, s.tokens, '卖点');
    for (const s of scientistCodes)
        push(s.model, s.tokens, '科学家代号');
    // FAQ models: deduplicate by category+modelSlug combo to avoid blowing up list
    const faqModelSeen = new Set();
    for (const f of faqs) {
        const k = `${f.category}:${f.modelSlug}`;
        if (faqModelSeen.has(k))
            continue;
        faqModelSeen.add(k);
 ...
 */
export function findModels(index, query, topN = 8) {
    ...
    if (qTokens.size > 0) {
        for (const m of index.allModels) {
            ...
            if (s > 0)
                modelHits.push({ name: m.name, score: s, origin: m.origin });
        }
    }
    modelHits.sort((a, b) => b.score - a.score);
    const dedup = new Map();
    for (const m of errorHits)
        dedup.set(`${m.origin}:${m.name}`, m);
 ...
 */
export function searchFaqs(index, { model, query, category, limit = 20, }) {
    if (!index.faqs || index.faqs.length === 0)
        return [];
    const qTrim = (query ?? '').trim();
    const qTokens = qTrim ? normalizeTokens(qTrim) : new Set();
    const mTrim = (model ?? '').trim();
    const mTokens = mTrim ? normalizeTokens(mTrim) : new Set();
    const catLower = category?.trim().toUpperCase();
    const hits = [];
    for (const f of index.faqs) {
        if (catLower && f.category && f.category.toUpperCase() !== catLower)
            continue;
        ...
        let modelScore = 0;
        if (mTrim) {
            if (f.modelSlug && mTrim.toLowerCase().includes(f.modelSlug.toLowerCase()))
                modelScore = 1;
            else if (f.modelSlug)
                modelScore = Math.max(modelScore, tokenJaccard(mTokens, normalizeTokens(f.modelSlug)));
            if (f.model)
                modelScore = Math.max(modelScore, tokenJaccard(mTokens, normalizeTokens(f.model)));
        }
        else {
...
export function freeSearch(index, query, topN = 8) {
    if (!query.trim())
        return [];
    ...
    for (const c of index.comparisons) {
        for (const [model, specs] of Object.entries(c.flatSpecs)) {
            ...
            for (const s of specs)
                pieces.push(`${s.section} ${s.spec} ${s.value}`);
