...
2. When the robot returns to the station for charging, the suction power will increase by itself. 
...
2. In Housekeeper Mode, the robot will automatically determine the suction power according to the room type. The suction power may be increased or decreased when working in different room types. 
...
Robot makes loud or abnormal noise when mopping or cleaning the mopping pads? 
1. Robot makes abnormal noise when mopping: 
(1) When the robot is mopping, the gears will make some noise when the mop motor is operating. 
(2) When the robot returns to the station, the mopping pad plates will spin faster for a smoother ride, resulting in the robot making some noise. 
All of the above noise produced during operation are normal. If you notice some other abnormal noise (e.g. clicks) also check whether the mopping pad plates are entangled with debris, and if so clean them promptly. 
2. The robot is loud when cleaning the mopping pads at the station: 
The reason is that when the robot is cleaning the mopping pads in the cleaning sink of the station, the grooves and wiper strip will make contact with the mopping pads in order to make the mopping pads cleaner. At the same time, the mopping pad plates will also be spinning at a relatively high speed, and this will generate some noise which is normal.
